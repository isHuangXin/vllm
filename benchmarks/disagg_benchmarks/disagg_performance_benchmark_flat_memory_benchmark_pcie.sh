...
kill_gpu_processes() {
  ...
  for port in 8000 8100 820014579 14580 30001; do lsof -t -i:$port | xargs -r kill -9; done
  sleep 1
}
...
launch_disagg_prefill() {
  ...
  # disagg 
  NCCL_IB_HCA=mlx5_2:1 CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES=0 vllm serve $model \
    --host 127.0.0.1 \
    --port 8100 \
    --max-model-len 10000 \
    --gpu-memory-utilization 0.6 \
